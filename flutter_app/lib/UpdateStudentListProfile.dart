...
import 'package:flutter_app/DataClass/StudentProfileData.dart';

class UpdateStudentListProfile extends StatefulWidget {
  final String stdUserId;

  UpdateStudentListProfile({Key key, this.stdUserId}) : super(key: key);

  @override
  _UpdateStudentListProfileState createState() =>
      new _UpdateStudentListProfileState();
}

String _radioValue;

class _UpdateStudentListProfileState extends State<UpdateStudentListProfile> {
  ...
  String _teacherId;
  String _schoolId;
  String _mySelectionClass;
  String _mySelectionSection;
  var _pickedImage;
  String mStrId = '';
  static bool isCheck;
  ...
  Future _getStudentProfile() async {
    try {
      http.Response response = await http.post(
          'http://182.72.79.154/p1136/ws_student_getMyProfile', body: {
        'student_id': widget.stdUserId
      });
      if (response.statusCode == 200) {
        setState(() {
          _studentProfileData =
          new StudentProfileData.fromJson(json.decode(response.body));
        });
      }
    } catch (err) {
      return err.toString();
    ...
    setState(() {
      _middleNameController.text = _studentProfileData.stdProfileUserMiddleName;
      _firstNameController.text = _studentProfileData.stdProfileUserFirstName;
      _lastNameController.text = _studentProfileData.stdProfileUserLastName;
      _emailController.text = _studentProfileData.stdProfileUserEmail;
      _addressOneController.text = _studentProfileData.stdUserProfileAddressOne;
      _addressTwoController.text = _studentProfileData.stdUserProfileAddressTwo;
      _contactOneController.text = _studentProfileData.stdUserProfileContactOne;
      _contactTwoController.text = _studentProfileData.stdUserProfileContactTwo;
      _dateOfBirthController.text = _studentProfileData.stdUserProfileDOB;
      _parentNameController.text = _studentProfileData.stdUserProfileParentName;

      _radioValue = _studentProfileData.stdUserProfileGender;
      // _mySelectionClass = _studentProfileData.stdUserProfileClassId;
      // _mySelectionSection = _studentProfileData.stdProfileUserSectionId;
    });
  }
  ...
  Future _updateStudentAndParentProfile() async {
    try {
      http.Response response = await http.post(
          'http://182.72.79.154/p1136/ws_student_editMyProfile', body: {
        'student_id': '',
        'school_id': _schoolId,
        ...
        'user_picture': '',
        'subject_id': mStrId,
        'section_id': _mySelectionSection
      });
      if (response.statusCode == 200) {
  ...
  void _handleSubmittedData() {
    ...
    if (!formState.validate()) {
      ...
      _updateStudentAndParentProfile();
    }
  }
                ...
                new Column(
                  children: <Widget>[
                    new DropdownButton<String>(
                      ...
                      value: _mySelectionClass,
                      onChanged: (String value) {
                        setState(() {
                          _mySelectionClass = value;
                ...
                new Column(
                  children: <Widget>[
                    new DropdownButton<String>(
                      ...
                      value: _mySelectionSection,
                      onChanged: (String value) {
                        setState(() {
                          _mySelectionSection = value;
                          _getClassSubjectList(value);
                        });
                      },
                      items: _mySelectionClass != null ?
